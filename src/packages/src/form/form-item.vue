<script lang="tsx">
  ...
  import type { NsFormItem, NsForm } from './types/form'

  import { defineComponent } from 'vue'
  import { FormItem } from 'ant-design-vue'
  import { cloneDeep, isBoolean, isFunction, isNull, isString, pick } from 'lodash-es'
  ...
  export default defineComponent({
    ...
    props: {
      ...
      type: vueTypes.string.def('normalForm')
    },

    setup(props, { slots }) {
      const realType = computed(() => props.formItem.component)
      //更加类型获取对应组件
      const realComponent = props.formItem.component
        ? (componentMap.get(props.formItem.component) as ReturnType<typeof defineComponent>)
        : null

      //获取对应组件属性
      const getComponentProps = computed(() => {
        const type = unref(realType) || ''
        ...
        const comOtherProps = Reflect.has(otherComponents, type)
          ...
          : []
        const propss = [...Object.keys(comProps), ...comOtherProps]
        return pick(props.formItem, propss || [])
      })
      ...
      function handleRules() {
        ...
        function validator(rule: any, value: any) {
          ...
          if (value === undefined || isNull(value) || (isString(value) && value.trim() === '')) {
      ...
      function renderLabelHelpMessage() {
        const { label, subLabel, tipsPlacement = 'label', component } = props.formItem
        ...
        return (
          <span>
            {renderLabel}
            {(tipsPlacement === 'label' || component === 'Divider') && renderTips()}
          </span>
        )
      }

      //v-model绑定checked的组件
      const checkedComponents = ['Checkbox', 'Switch']
      ...
      const { formItem, formProps } = toRefs(props) as {
        formItem: Ref<NsFormItem>
        formProps: Ref<NsForm>
      }
      ...
      return () => {
        ...
        return (
          ...
          (formItem.component === 'Divider' ? (
            ...
            <FormItem
              v-show={isShow.value}
              {...getFormItemProps.value}
              rules={handleRules()}
              label={renderLabelHelpMessage()}
              labelCol={labelCol}
              wrapperCol={wrapperCol}>
              <div class="flex items-center">
                <div class="flex-1 ns-input_wraps">{getInputComponentItem()}</div>
